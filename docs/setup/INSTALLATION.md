...
| --- | --- | --- | --- |
| Python | 3.11+ | `python3 --version` / `python --version` | https://www.python.org/downloads/ |
| Node.js | 18+ | `node --version` | https://nodejs.org/ |
| pnpm | 9+ | `pnpm --version` | `npm install -g pnpm` or https://pnpm.io/installation |
...
sudo dnf install -y webkit2gtk4.1-devel openssl-devel curl wget file \
  libappindicator-gtk3-devel librsvg2-devel gtk3-devel @"C Development Tools and Libraries"
...
Miori reads configuration from `.env` files. Copy the templates and adjust.
...
The fastest path is the bootstrap script — it creates the backend venv, installs
`requirements.txt`, and runs `pnpm install` for the frontends.
...
Before (or after) bootstrap, you can sanity-check your environment:
...
Install it globally: `npm install -g pnpm` (or see https://pnpm.io/installation).
Verify with `pnpm --version` (need 9+).
